    ...
    if closestDist and closestDist < 10 then
      if not lastPlate or closest.vehProps.plate ~= lastPlate then
        ...
        if closest.owner ~= playerData.identifier then
          drawText = "[~r~"..GetDisplayNameFromVehicleModel(closest.vehProps.model).."~s~] Appuyer sur [~r~E~s~] pour acheter  [$~r~"..closest.price.."~s~]"
        else
          drawText = "[~r~"..GetDisplayNameFromVehicleModel(closest.vehProps.model).."~s~] Appuyer sur [~r~E~s~] pour récupérer votre véhicule [$~r~"..closest.price.."~s~]"
        end
        local turbs = 'No'
      ...
      if IsControlJustPressed(0,38) and closestDist < 5.0 and GetGameTimer() - lastTimer > 150 then
        ...
        if not isConfirming then
          if closest.owner ~= playerData.identifier then
            drawText = "[~r~"..GetDisplayNameFromVehicleModel(closest.vehProps.model).."~s~] Appuyer sur [~r~E~s~] à nouveau pour confirmer cet achat  [$~r~"..closest.price.."~s~]"
          else
            lastPlate = false
            BuyVehicle(closest)
          end
...
function SellCar(price)
  if not price or not price[1] then ESX.ShowNotification("Vous devez entrer un prix ."); return; end
  if type(price) == "table" then price = tonumber(price[1]); end
  if not price or type(price) ~= "number" or price <= 0 then ESX.ShowNotification("Arrête de baisser le prix ."); return; end
  if not IsPedInAnyVehicle(GetPlayerPed(-1),false) then ESX.ShowNotification("Vous devez être dans un véhicule. "); return; end
  if not sellAnywhere and GetVecDist(GetEntityCoords(GetPlayerPed(-1)),salesYard) > salesRadius then ESX.ShowNotification("Vous devez être sur le chantier pour faire cela ."); return; end
