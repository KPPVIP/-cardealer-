...
local venta = { x = 1210.38, -3105.35, 5.88}
...
function OpenShop()
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'car_shop', {
		...
		exports.pNotify:SendNotification({text = "Go flip the truck", type = "success", timeout = 3000, layout = "centerRight", queue = "right", animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
...
function ChopVehicle()
    ...
    if seats ~= 0 then
        TriggerEvent('chat:addMessage', { args = { 'There can be no passengers' } })
    elseif
        ...
        ESX.TriggerServerCallback('esx_chatarrero:anycops', function(anycops)
            --if anycops >= Config.CopsRequired then
                ...
                if not HasAlreadyEnteredMarker then
                    ...
                    exports.pNotify:SendNotification({text = "You didn't let the junkyards finish.", type = "error", timeout = 4000, layout = "centerRight", queue = "right", killer = true, animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        ...
        exports.pNotify:SendNotification({
            text = "Puedes volver en " ..math.floor(timerNewChop / 60000).. " minute",
            type = "error",
            timeout = 1000,
            layout = "centerRight",
            queue = "right",
...
function VehiclePartsRemoval()
	...
	puerta1 = CreatePed(1, Config.NPCHash1, 1206.42, -3117.25, 4.54, 48.98, false, true)
	...
	--TaskPlayAnim(puerta1, "timetable@gardener@filling_can", "gar_ig_5_filling_can", 2.0, 8.0, -1, 50, 0, 0, 0, 0)
        ...
        exports['progressBars']:startUI(Config.DoorOpenFrontLeftTime, "Opening the front left door")
        Citizen.Wait(Config.DoorOpenFrontLeftTime)
        SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 0, false, false)
    end
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
        exports['progressBars']:startUI(Config.DoorBrokenFrontLeftTime, "Removing the front left door")
    ...
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
	puerta2 = CreatePed(1, Config.NPCHash1, 1203.20, -3117.56, 4.54, 319.86, false, true)
	SetCurrentPedWeapon(puerta2, -2067956739, true)
	TaskStartScenarioInPlace(puerta2, "PROP_HUMAN_BUM_BIN", 0, 1)
        exports['progressBars']:startUI(Config.DoorOpenFrontRightTime, "Opening the front right door")
        Citizen.Wait(Config.DoorOpenFrontRightTime)
        SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 1, false, false)
    end
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
        exports['progressBars']:startUI(Config.DoorBrokenFrontRightTime, "Removing the front right door")
    ...
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
	puerta3 = CreatePed(1, Config.NPCHash1, 1206.57, -3115.97, 4.54, 57.90, false, true)
	SetCurrentPedWeapon(puerta3, -2067956739, true)
	TaskStartScenarioInPlace(puerta3, "PROP_HUMAN_BUM_BIN", 0, 1)
        exports['progressBars']:startUI(Config.DoorOpenRearLeftTime, "Opening the left rear door")
    ...
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
        exports['progressBars']:startUI(Config.DoorBrokenRearLeftTime, "Removing the left rear door")
    ...
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
		puerta4 = CreatePed(1, Config.NPCHash1, 1203.07, -3116.22, 4.54, 307.09, false, true)
	SetCurrentPedWeapon(puerta4, -2067956739, true)
	TaskStartScenarioInPlace(puerta4, "PROP_HUMAN_BUM_BIN", 0, 1)
        exports['progressBars']:startUI(Config.DoorOpenRearRightTime, "Opening the right rear door")
        Citizen.Wait(Config.DoorOpenRearRightTime)
        SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 3, false, false)
    end
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
        exports['progressBars']:startUI(Config.DoorBrokenRearRightTime, "Removing the right rear door")
    ...
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
	capo = CreatePed(1, Config.NPCHash1, 1204.96, -3119.15, 4.54, 2.86, false, true)
    ...
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
        exports['progressBars']:startUI(Config.DoorBrokenHoodTime, "Removable hood")
    ...
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
	trunk = CreatePed(1, Config.NPCHash1, 1204.69, -3112.80, 4.54, 177.56, false, true)
	SetCurrentPedWeapon(trunk, -2067956739, true)
	TaskStartScenarioInPlace(trunk, "PROP_HUMAN_BUM_BIN", 0, 1)
        exports['progressBars']:startUI(Config.DoorOpenTrunkTime, "Opening the trunk")
        Citizen.Wait(Config.DoorOpenTrunkTime)
        SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 5, false, false)
    end
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
        exports['progressBars']:startUI(Config.DoorBrokenTrunkTime, "Disassembly of the trunk")
        Citizen.Wait(Config.DoorBrokenTrunkTime)
		DeleteEntity(trunk)
        SetVehicleDoorBroken(GetVehiclePedIsIn(ped, false),5, true)
    end
    Citizen.Wait(1000)
    exports['progressBars']:startUI(Config.DeletingVehicleTime, "End of disposal")
    Citizen.Wait(Config.DeletingVehicleTime)
    if ChoppingInProgress == true then
        DeleteVehicle()
		TriggerServerEvent("esx_chatarrero:piezas")
		TriggerServerEvent("esx_chatarrero:llanta")
        exports.pNotify:SendNotification({text = "Scrapped vehicle, now take this truck and go sell the parts. If you don't want to sell the parts, turn the truck over", type = "success", timeout = 5000, layout = "centerRight", queue = "right", animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
		Spawn()
	SetNewWaypoint(venta.x,venta.y)
	final = CreatePed(1, Config.NPCHash1, 1204.69, -3112.80, 4.54, 177.56, false, true)
	SetCurrentPedWeapon(capo, -2067956739, true)
...
function MotoPartsRemoval()
    ...
    SetVehicleEngineOn(vehicle, false, false, true)
    SetVehicleUndriveable(vehicle, false)
    exports['progressBars']:startUI(Config.DeletingVehicleTime, "The demolition of the motorcycle")
    Citizen.Wait(Config.DeletingVehicleTime)
    if ChoppingInProgress == true then
        DeleteVehicle()
		TriggerServerEvent("esx_chatarrero:moto")
        exports.pNotify:SendNotification({text = "Scrapped vehicle, you just have to give your parts to a mechanic so that he can go and sell them..", type = "success", timeout = 5000, layout = "centerRight", queue = "right", animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
		Spawn()
	SetNewWaypoint(venta.x,venta.y)
	final = CreatePed(1, Config.NPCHash1, 1204.69, -3112.80, 4.54, 177.56, false, true)
	SetCurrentPedWeapon(capo, -2067956739, true)
...
function Spawn()
	...
	local spawned_car = CreateVehicle(vehicle, spawncamion.x,spawncamion.y,spawncamion.z, spawncamion.h, -0.0, -0.0, 0.12, 0.16, true, false)
...
AddEventHandler('esx_chatarrero:hasExitedMarker', function(zone)
    ...
    if zone == 'Chopshop' then
        ...
        if ChoppingInProgress == true then
            exports.pNotify:SendNotification({text = "You turned away from the junkyards.", type = "error", timeout = 1000, layout = "centerRight", queue = "right", killer = true, animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        end
    end
    ChoppingInProgress        = false
...
Citizen.CreateThread(function()
    local hash = GetHashKey("csb_hugh")
    while not HasModelLoaded(hash) do
    RequestModel(hash)
    Wait(20)
    end
    ped = CreatePed("PED_TYPE_CIVFEMALE", "csb_hugh", 561.36, -2831.61, 5.02, 155.16, false, true)
		...
		local playerPed = PlayerPedId()
         DrawMarker(1, Config.Spawn.unspawn.coords.x, Config.Spawn.unspawn.coords.y, Config.Spawn.unspawn.coords.z, 0, 0, 0, 0, 0, 0, 1.5001, 1.5001, 0.6001,255,255,51, 200, 0, 0, 0, 0)
          ...
          if GetDistanceBetweenCoords(coords, Config.Spawn.unspawn.coords, true) < 1.5 then
            ...
            ESX.ShowHelpNotification(('Press E to save the truck.'))
	        ...
	        if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0x85A5B471)  then
			 ...
			 Citizen.Wait(5000)
			  ...
			  exports.pNotify:SendNotification({text = "This vehicle is not the work truck", type = "error", timeout = 2000, layout = "centerRight", queue = "right", killer = true, animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
			end
        end
    end
    end
