...
if GetCurrentResourceName() == 'esx_chatarrero' then
  ...
  print("\n".. " Correctly loaded. Published by WackySpring and Ikarus  ")
	...
	math.randomseed(GetGameTimer())
		...
		TriggerClientEvent('esx:showNotification', source, ('You already have all the doors.'))
	else
	...
	if xItem.limit ~= -1 and (xItem.count + 1) > xItem.limit then
		TriggerClientEvent('esx:showNotification', source, ('You already have a battery'))
	else
   xPlayer.addInventoryItem('bateria',math.random(0, 1))
		...
		TriggerClientEvent('esx:showNotification', source, ('You cannot carry more tires'))
	else
      xPlayer.addInventoryItem('llanta',math.random(1, 2))
		end
	
			if xItem2.limit ~= -1 and (xItem2.count + 1) > xItem2.limit then
		TriggerClientEvent('esx:showNotification', source, ('You cannot carry more radios'))
	else
	...
	if xItem.limit ~= -1 and (xItem.count + 1) > xItem.limit then
		TriggerClientEvent('esx:showNotification', source, ('Vous avez déjà une cagoule.'))
	else
	...
	if xItem.limit ~= -1 and (xItem.count + 1) > xItem.limit then
		TriggerClientEvent('esx:showNotification', source, ('Vous avez déjà une batterie'))
	else
   xPlayer.addInventoryItem('bateria',math.random(0, 1))
		end
		...
		if xItem1.limit ~= -1 and (xItem1.count + 1) > xItem1.limit then
		TriggerClientEvent('esx:showNotification', source, ('Vous ne pouvez pas transporter plus d\'airbags'))
	else
      xPlayer.addInventoryItem('airbag',math.random(1, 2))
		end
	
			if xItem2.limit ~= -1 and (xItem2.count + 1) > xItem2.limit then
		TriggerClientEvent('esx:showNotification', source, ('Vous ne pouvez pas transporter plus de radios'))
	else
       xPlayer.addInventoryItem('radio',math.random(0, 1))
	end
	...
	if xItem.limit ~= -1 and (xItem.count + 1) > xItem.limit then
		TriggerClientEvent('esx:showNotification', source, ('Vous avez déjà tous les pneus.'))
	else
