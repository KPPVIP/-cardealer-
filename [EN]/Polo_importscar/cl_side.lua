...
RMenu:Get('mission', 'main'):SetSubtitle("~b~Mission menu")
...
Citizen.CreateThread(function()
    ...
    for _,v in pairs(configPed) do
        ...
        if v.legal then
		    ...
		    AddTextComponentSubstringPlayerName("~b~Citizen Mission")
    ...
    while true do
        ...
        for _,v in pairs(configPed) do
            ...
            if dst <= 1.0 then
                ShowHelpNotification("To push on~INPUT_PICKUP~ to open the mission menu")
            ...
            RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()
                for _,v in pairs(configMission) do
                    if v.ped == id then
                        ...
                        if not done then
                            RageUI.Button(v.Titre.." | ~b~"..v.prix.."~w~$ - ~g~AVAILABLE", v.Desc, {}, true, function(Hovered, Active, Selected)
                                if (Selected) then
                            ...
                            RageUI.Button(v.Titre.." - ~r~UNAVAILABLE", nil, { RightBadge = RageUI.BadgeStyle.Tick }, true, function(Hovered, Active, Selected)end)
                        end
                    end
                end
            end, function()
